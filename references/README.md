...
This directory holds extracted and structured versions of reference works used in the Ghandwa project. PDFs are stored alongside their extraction packages where available.
...
Known gaps: paradigm cells in Chs. 2 and 4 need spot-check against PDF before citation; prose-in-fence artifacts and paradigm cell spacing artifacts pending cleanup (see `ringe-2017-cleanup-handoff.md`).
...
### Van Sluis, Jørgensen & Kroonen 2023
...
Roles: `DIAC` `SUB`  
Extraction: complete, second pass.  
...
### Hyllested 2010
**Adam Hyllested. "Precursors to Celtic and Germanic." [conference paper/chapter]**  
Roles: `SUB`  
Extraction: PDF only (Xerox scan, 22 pp., landscape). ChatGPT extraction in progress.  
...
Roles: `LEX`  
Extraction: front matter and introduction only (PDF pages 1–27). Dictionary proper not yet extracted.  
...
See `matasovic-2009-frontmatter.md`.
...
Roles: `LEX` `DIAC`  
Extraction: targeted — pp. 175–179 only (Ch. 29, athematic nominal suffixes, n-stems). Full extraction pending (bad scan; OCR quality is a risk for diacritic-heavy notation).  
...
| Date | Change |
|---|---|
| 2026-04-27 | Initial README created. Inventory covers all files present in `references/` as of commit `07d7540`. |
