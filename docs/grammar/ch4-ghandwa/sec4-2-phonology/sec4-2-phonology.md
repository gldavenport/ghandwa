---
title: "§4.2 Phonology"
last_updated: 2026-05-30T00:00-04:00
chapter: ch4-ghandwa
section: sec4-2-phonology
supersedes: "docs/grammar/ch3-development/phonological-history.md (§§1, 6, 7)"
---

# §4.2 Phonology

## §4.2.1 The Ghandwa Phonological Inventory

The Ghandwa surface inventory, as derived by the rules of Chapter 3:

**Stops:** p  b  t  d  k  g  kʷ  gʷ
**Fricatives:** s  z  β  ð  ɣ  ɣʷ
**Nasals:** m  n
**Liquids:** l  r
**Glides:** w  j
**Short vowels:** a  e  i  o  u
**Long vowels:** ā  ē  ī  ō  ū

Vowel length is phonemically distinctive. Stress is free and lexical. Labiovelars kʷ, gʷ are preserved before vowels other than *u*; they delabialized before *u*, *ū*, *w*, and before consonants (see §4.2.2).

The four voiced fricatives β, ð, ɣ, ɣʷ are the defining Ghandwa phonemes. They arise historically from the PIE aspirated stops (§3.2.3.3) and remain stable at the Ghandwa stage. They constitute a deliberate phonological tension for daughter languages, which may devoice, stop, or merge them independently.

## §4.2.2 Consonant Alternations: Labiovelar Standing Rules

Two rules function as standing surface constraints. They apply after every stage of the historical derivation and reapply until convergence; they also remain productive synchronic phonotactics of Ghandwa. For their diachronic origins see §3.2.1.

### §4.2.2.1 Labiovelar Absorption

> Kw → Kʷ

Any surface sequence of velar + w is reanalyzed as a labiovelar. This rule feeds §4.2.2.2 and operates in both derived and non-derived environments.

### §4.2.2.2 The Boukólos Rule

> Kʷ → K / \_ {C, w, u, ū} (C excludes syllabic resonants)

Labiovelars delabialize before any consonant, or before w/u/ū. The rule is **forward-only** — it is not triggered by a preceding rounded segment — and runs post-Stage 1 and post-Stage 2, never at the PIE-internal pre-stage. This restriction is historically important: it means the rule applies to derived environments that the pre-stage PIE-internal delabialization (§3.2.1.3) could not have seen.

The Boukólos rule creates systematic stem alternations in paradigms where the following vowel varies between rounded and unrounded. The o-stem noun \*perkʷus ~ \*perkʷéws 'oak' illustrates this:

- Nominative *perkus*: kʷ before u → delabializes
- Genitive *perkʷews*: kʷ before e → preserved

The resulting kʷ ~ k allomorphy is a **deliberate unresolved tension** (see §4.2.3). Daughter languages may level it in either direction.

The interaction with labiovelar absorption (§4.2.2.1) is important: absorption can create new labiovelars from velar+w sequences, which the Boukólos rule then feeds on. Both rules must be run to convergence in sequence (absorption → Boukólos) at each stage.

## §4.2.3 Deliberate Phonological Tensions

A central design principle of Ghandwa is that not every phonological instability is resolved at the parent-language stage. The following features are left as tensions that daughter languages are expected to resolve independently:

| Feature | Status in Ghandwa | Expected daughter variation |
|---|---|---|
| *kt* clusters (from thorn metathesis, §3.2.2.2) | Stable | Daughters may simplify via assibilation, fricativization, or loss of one member |
| Labiovelar stem allomorphy kʷ ~ k (from §4.2.2.2) | Not leveled | Daughters may generalize either alternant |
| Voiced fricatives β, ð, ɣ, ɣʷ | Stable | Daughters may devoice, stop, or merge them |
| Labiovelars before front vowels | Preserved | Daughters may palatalize, labialize, or simplify |
| Nasal + obstruent clusters (np, nk, nt, etc.) | No nasal assimilation | Daughters may assimilate |
| Non-high VV hiatus from laryngeal loss | Unresolved (a.o, e.a, etc.) | Daughters may contract, insert glides, or tolerate hiatus |
| *sr* clusters | Preserved | Daughters may dissimilate (sr → θr, as in Italic/Celtic) |
| *βu-* onset | Preserved | Flagged as a likely site of phonological mutation in daughter languages |
